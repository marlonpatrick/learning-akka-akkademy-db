...
public class TellDemoArticleParser extends AbstractActor {
    ...
    private ActorRef buildExtraActor(ActorRef senderRef, String uri) {
        ...
        class HandleResponseActor extends AbstractActor {

            @Override
            public Receive createReceive() {

                return receiveBuilder().
                        matchEquals("timeout", x -> { //if we get timeout, then fail
                            senderRef.tell(new Status.Failure(new TimeoutException("timeout!")), self());
                            context().stop(self());
                        })
                        .match(KeyNotFoundException.class, ex -> {
                            httpClientActor.tell(uri, self());
                        })
        ...
        return context().actorOf(Props.create(HandleResponseActor.class, () -> new HandleResponseActor()));
    }
}